...
var TypeResolver = /** @class */ (function () {
        ...
        if (this.typeNode.kind === ts.SyntaxKind.UnionType) {
            ...
            if (supportType) {
                ...
                var types = this.typeNode.types.map(function (type) {
                    return new TypeResolver(type, _this.current, _this.parentNode, _this.extractEnum).resolve();
                });
                return {
                    dataType: 'union',
                    types: types,
    ...
    TypeResolver.prototype.getModelProperties = function (node, genericTypes) {
        ...
        if (classConstructor && classConstructor.parameters) {
            var constructorProperties = classConstructor.parameters
                .filter(function (parameter) { return _this.isPublicParameter(parameter); });
    ...
    TypeResolver.prototype.isPublicParameter = function (node) {
        return node.modifiers && node.modifiers.some(function (modifier) { return modifier.kind === ts.SyntaxKind.PublicKeyword; });
    };
